...
export default function ClickBattle() {
  ...
  useEffect(() => {
    if (!profile && view === "company") return;
    fetchBoard(view);
  }, [view, profile]);
  ...
  const hit = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    setTimeout(() => setParticles((p) => p.filter((r) => r.id !== id)), 800);
  };
    ...
    return (
      <main className="min-h-screen bg-slate-950 text-white relative overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div
            className={`absolute rounded-full border transition-all duration-1000 ${isUrgent ? "border-red-500/20" : "border-blue-500/10"}`}
            style={{ width: `${200 + (10 - time) * 40}px`, height: `${200 + (10 - time) * 40}px` }}
          />
          <div
            className={`absolute rounded-full border transition-all duration-1000 ${isUrgent ? "border-red-500/10" : "border-blue-500/5"}`}
            style={{ width: `${350 + (10 - time) * 50}px`, height: `${350 + (10 - time) * 50}px` }}
          />
          <div
            className={`absolute rounded-full transition-all duration-1000 ${isUrgent ? "bg-red-500/5" : "bg-blue-500/5"}`}
            style={{ width: `${500 + (10 - time) * 30}px`, height: `${500 + (10 - time) * 30}px` }}
          />
        </div>
        ...
        <div className="max-w-sm mx-auto pt-16 px-6 text-center relative z-10">
          ...
          <div className="mb-6">
            <div className="flex justify-between text-sm mb-2">
              ...
              <span className={`font-bold ${isUrgent ? "text-red-400" : "text-white"}`}>{time}s</span>
            </div>
          ...
          <div className="mb-10">
            ...
            <p
              className={`font-black transition-all duration-100 ${clickAnim ? "scale-125 text-blue-300" : "scale-100 text-white"}`}
          ...
          <div className="relative">
            ...
            <button
              ...
              className={`w-full py-14 text-3xl font-black rounded-3xl transition-all duration-75 relative overflow-hidden ${
                clickAnim
                  ? isUrgent ? "bg-red-500 scale-95" : "bg-blue-400 scale-95"
